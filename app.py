...
with st.sidebar:
    st.header("⚙️ 프로젝트 설정")
    project_name = st.text_input("프로젝트명", "OO교량 정밀점검")
    inspector = st.text_input("진단자", "홍길동")
    st.divider()
    st.caption("시스템 버전: v2.5")
...
with tab1:
    ...
    if st.button("평가 실행", type="primary", key="btn_carb", use_container_width=True):
        ...
        with st.container(border=True):
            ...
            st.info(f"""
            **상세 계산 근거:**
            1. **탄산화 속도계수(A)**: $A = C / \\sqrt{{t}} = {m_depth} / \\sqrt{{{a_years}}} = {rate_a:.3f}$ $mm/\\sqrt{{year}}$
            2. **철근 도달 예상 시간(T)**: $T = (Cover / A)^2 = ({d_cover} / {rate_a:.3f})^2 = {total_life:.1f}$ 년
            3. **최종 잔여수명**: $T - t = {total_life:.1f} - {a_years} = {res_life:.1f}$ 년
            """)
...
with tab2:
    ...
    if mode == "단일 입력":
        with st.container(border=True):
            ...
            txt = st.text_area("측정값 (공백/줄바꿈으로 구분)", "54 56 55 53 58 55 54 55 52 57 55 56 54 55 59 42 55 56 54 55", height=100)
        ...
        if st.button("계산 실행", type="primary", use_container_width=True):
            ...
            if not success:
                ...
                st.success(f"평균 추정 압축강도: **{res['Mean_Strength']:.2f} MPa** (설계비: {(res['Mean_Strength']/fck*100):.1f}%)")
                ...
                with col_right:
                    ...
                    st.caption("※ 빨간 점선은 설계강도 기준선입니다.")

                with st.expander("🚫 이상치 기각 상세 내역"):
                    st.write(f"**1차 측정 평균**: {res['R_initial']:.2f}")
                    st.write(f"**기각된 데이터 개수**: {res['Discard']}개")
                    if res['Excluded']:
                        st.write(f"**기각된 값 리스트**: {res['Excluded']}")
                    else:
                        st.write("기각된 데이터가 없습니다.")

    elif mode == "다중 입력 (Batch)":
        st.info("엑셀 데이터를 아래 표에 붙여넣거나 직접 편집할 수 있습니다.")
        batch_txt = st.text_area("데이터 붙여넣기 (지점 각도 재령 설계강도 측정값...)", height=100, placeholder="P1 0 1000 24 55 56 54 ...")
        ...
        if batch_txt.strip():
            for line in batch_txt.strip().split('\n'):
                p = line.split()
                if len(p) > 5:
                    init_data.append({"선택":True, "지점":p[0], "각도":int(p[1]), "재령":int(p[2]), "설계":float(p[3]), "데이터":" ".join(p[4:])})
        
        df_batch = pd.DataFrame(init_data) if init_data else pd.DataFrame(columns=["선택","지점","각도","재령","설계","데이터"])
        edited_df = st.data_editor(df_batch, use_container_width=True, hide_index=True)
        ...
        if st.button("🚀 일괄 계산 실행", type="primary", use_container_width=True):
            ...
            for _, row in edited_df.iterrows():
                ...
                if ok:
                    batch_results.append({
                        "지점": row["지점"], "R0": round(res["R0"],1), "Alpha": round(res["Age_Coeff"],2),
                        "설계": row["설계"], "평균강도": round(res["Mean_Strength"],2), "강도비(%)": round((res["Mean_Strength"]/row["설계"])*100,1)
                    })
            ...
            if batch_results:
                ...
                # 결과 차트
                b_base = alt.Chart(res_df).encode(x=alt.X('지점', sort=None))
                b_bars = b_base.mark_bar().encode(y='평균강도', color=alt.condition(alt.datum.평균강도 >= alt.datum.설계, alt.value('#4D96FF'), alt.value('#FF6B6B')))
                b_ticks = b_base.mark_tick(color='red', thickness=3, size=30).encode(y='설계')
                st.altair_chart(b_bars + b_ticks, use_container_width=True)
...
with tab3:
    ...
    if st.button("통계 분석 실행", key="btn_st_run", use_container_width=True):
        ...
        if len(data) >= 2:
            ...
            st.metric("종합 평균 강도", f"{avg_val:.2f} MPa", delta=f"{(avg_val/st_fck*100):.1f}% (설계 대비)")
            ...
            st.altair_chart(s_chart + s_rule, use_container_width=True)
            
            cols = st.columns(3)
            cols[0].metric("최대값", f"{max(data):.2f}")
            cols[1].metric("최소값", f"{min(data):.2f}")
            cols[2].metric("변동계수(CV)", f"{(np.std(data, ddof=1)/avg_val*100):.1f}%")
